...
`site/` is the selected public presentation. `pnpm build` renders `_context/why-origin-stories.md` into the site and copies only `site/` into `dist/`. Publish only `dist/`, never the repository root. Pushes to `main` trigger GitHub Pages publication. The essay is a working draft in Marlon’s first-person perspective for revision. The repository is public: tracked editorial sources in `_context/` are visible on GitHub even when they are not deployed as website pages. Never add private participant material there.
