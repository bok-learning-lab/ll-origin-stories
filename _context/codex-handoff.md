...
`site/` holds selected presentation art, portable HTML readers, the index, and rendered posts. `site/collection.json` records the current reading order and older short mutant editions. There are thirty current eight-panel comics: five immigrant/elsewhere stories, five Golden Age printed re-renderings of those same stories (2026-09-11, generated through the OpenRouter route with google/gemini-3.1-flash-image), five classic exposure stories, five mutant stories, four individual modern affluent antihero stories plus the ensemble resources parody, and four individual 1980s corporate-gothic villain origins plus The Institute for Your Own Good ensemble. The four earlier four-panel mutant origins remain in an expandable archive on the index.

`_context/comic-scripts/` contains exact scripts for all thirty current comics; the Golden Age scripts also carry the shared style block in `the-learning-lab-assemble-golden-age/golden-age-style.md`. `_context/why-origin-stories.md` and `_context/how-this-was-made.md` are editable post sources. The repository is public, including its tracked `_context/` files. Never put private participant information or credentials there. Full local production folders for the recent mutant expansions and resources parody are in ignored `output/comics/`; earlier full production runs remain in the original pack and Downloads. The modern antihero production is in `output/the-discretionary-class/`; the original villain ensemble is in `output/the-institute-for-your-own-good/`; the four individual villain origins plus the preserved ensemble are collected in `output/the-institute-secret-origins/`. These new pages used the built-in image tool; the underlying model was not exposed. The main index has separate cards and readers for all nine additions. Full scripts, exact prompts, originals and generation records remain in the ignored production folders. All selected likeness sheets needed to continue are already in the site folders.
